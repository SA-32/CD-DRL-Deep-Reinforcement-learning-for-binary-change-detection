...
def rollout_episode(model, env, aggregator, pre_change_image, post_change_image, gt, t_max, gamma, device):
    ...
    # NEW: per-timestep diagnostics
    action_zero_frac = []   # fraction of pixels choosing action==0 ("remove") at each t
    entropy_mean = []       # mean entropy at each t

    for t in range(t_max):
        x_t = torch.cat([pre_change_image, post_change_image, mask], dim=1)
        action, log_prob, entropy, value, _ = model.act(x_t, t)

        # NEW: log without breaking the graph (use .detach())
        action_zero_frac.append((action == 0).float().mean().item())
        entropy_mean.append(entropy.mean().item())
    ...
    # NEW: return the diagnostics too
    return log_probs, entropies, values, rewards, mask, mask_history, action_zero_frac, entropy_mean
...
def plot_diagnostics(action_history, entropy_history, epoch):
    """
    action_history / entropy_history: list of length num_epochs,
    each element a list of length t_max (per-timestep values for that epoch)
    """
    action_arr = np.array(action_history)   # (epochs, t_max)
    entropy_arr = np.array(entropy_history) # (epochs, t_max)

    fig, axes = plt.subplots(1, 2, figsize=(12, 5))

    # Per-timestep action fraction across epochs (one line per timestep)
    for t in range(action_arr.shape[1]):
        axes[0].plot(action_arr[:, t], label=f"t={t}")
    axes[0].set_title("Fraction of action==0 (remove) per timestep")
    axes[0].set_xlabel("Epoch")
    axes[0].set_ylabel("Fraction")
    axes[0].legend(fontsize=7, ncol=2)

    for t in range(entropy_arr.shape[1]):
        axes[1].plot(entropy_arr[:, t], label=f"t={t}")
    axes[1].set_title("Mean entropy per timestep")
    ...
    plt.savefig(f"rollouts/diagnostics_epoch_{epoch}.png")
    plt.close()
...
def train(cfg: Config,  k_shot=None):
    ...
    global_step = 0
    action_history = []   # will hold one [t_max] list per epoch
    entropy_history = []
    for epoch in range(1, 201):
        ...
        for batch_idx, (pre_change_image, post_change_image, gt) in enumerate(train_loader):
            ...
            log_probs, entropies, values, rewards, final_mask, mask_history, action_zero_frac, entropy_mean = rollout_episode(
            ...
            if batch_idx == 0:
                ...
                action_history.append(action_zero_frac)
                entropy_history.append(entropy_mean)
                plot_diagnostics(action_history, entropy_history, epoch)
        ...
        if epoch % 50 == 0:
            ckpt_path = f"pixeldrl_mg_epoch{epoch}.pt"
